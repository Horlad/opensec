import { afterEach, beforeEach, describe, expect, test } from "bun:test"
import fs from "fs/promises"
import os from "os"
import path from "path"
import { X509Certificate } from "crypto"
import { CertRuntime } from "../../src/cert/runtime"

const CA_PEM = `-----BEGIN CERTIFICATE-----

Naj4e1BsuDSJ7bgNvP4gD7NUKqCLi5ojuO8Z
-----END CERTIFICATE-----`

const LEAF_PEM = `-----BEGIN CERTIFICATE-----

5htG7iUpyvWEWzLsUcR6Zhvl
-----END CERTIFICATE-----`

const prevState = process.env.OPENCODE_COMMAND_CERT_STATE_FILE
const prevSkip = process.env.OPENCODE_COMMAND_CERT_SKIP_TRUST
const file = path.join(os.tmpdir(), `opencode-cert-runtime-${process.pid}.json`)

describe("cert.runtime", () => {
  beforeEach(async () => {
    process.env.OPENCODE_COMMAND_CERT_STATE_FILE = file
    process.env.OPENCODE_COMMAND_CERT_SKIP_TRUST = "1"
    await fs.rm(file, { force: true })
  })

  afterEach(async () => {
    await fs.rm(file, { force: true })
    if (prevState === undefined) delete process.env.OPENCODE_COMMAND_CERT_STATE_FILE
    if (prevState !== undefined) process.env.OPENCODE_COMMAND_CERT_STATE_FILE = prevState
    if (prevSkip === undefined) delete process.env.OPENCODE_COMMAND_CERT_SKIP_TRUST
    if (prevSkip !== undefined) process.env.OPENCODE_COMMAND_CERT_SKIP_TRUST = prevSkip
  })

  test("normalizes PEM and DER input to the same fingerprint", async () => {
    const pem = await CertRuntime.install(CA_PEM)
    await CertRuntime.remove(pem.entry.fingerprint)

    const der = Buffer.from(new X509Certificate(CA_PEM).raw)
    const fromDer = await CertRuntime.install(der)

    expect(fromDer.entry.fingerprint).toBe(pem.entry.fingerprint)
    expect(fromDer.entry.pem.startsWith("-----BEGIN CERTIFICATE-----")).toBe(true)
    expect(fromDer.entry.pem.endsWith("-----END CERTIFICATE-----\n")).toBe(true)
  })

  test("rejects non-CA certificate input", async () => {
    await expect(CertRuntime.install(LEAF_PEM)).rejects.toThrow("Certificate is not a CA certificate")
  })

  test("deduplicates by fingerprint", async () => {
    const first = await CertRuntime.install(CA_PEM)
    const second = await CertRuntime.install(CA_PEM)

    expect(second.entry.fingerprint).toBe(first.entry.fingerprint)
    expect(second.runtime.applied).toBe(false)
    expect((await CertRuntime.list()).length).toBe(1)
  })

  test("updates persisted state on add and remove", async () => {
    const added = await CertRuntime.install(CA_PEM)
    expect((await CertRuntime.list()).map((item) => item.fingerprint)).toEqual([added.entry.fingerprint])
    expect(await Bun.file(file).exists()).toBe(true)

    const removed = await CertRuntime.remove(added.entry.fingerprint)
    expect(removed.entry.fingerprint).toBe(added.entry.fingerprint)
    expect(await CertRuntime.list()).toEqual([])
    expect(await Bun.file(file).exists()).toBe(false)
  })
})
